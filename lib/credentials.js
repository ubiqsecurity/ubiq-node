...
class Credentials {
  ...
  isIdp() {
    if (verbose) console.log(`credentials isIdp: started`);
    if (verbose) console.log(`credentials isIdp idp_username: empty?(${!this.idp_username})`);
    let ret = (!this.idp_username) ? false : (this.idp_username.length > 0)
    if (verbose) console.log(`credentials isIdp: ret : (${ret})`);
