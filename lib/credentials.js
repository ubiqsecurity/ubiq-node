...
class Credentials {
  constructor(access_key_id, secret_signing_key, secret_crypto_access_key, host, idp_username, idp_password) {
    ...
    // return (set_attributes(this.access_key_id, this.secret_signing_key, this.secret_crypto_access_key, this.host, this.idp_username, this.idp_password));
  ...
  async initAsync(ubiqConfiguration) {
    if (this.idp_username) {
      ...
      this.csr = await generateCsr(this.access_key_id, apiPublicKey_pem, apiPrivateKey_pem)
