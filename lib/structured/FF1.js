...
class FF1 extends FFX {
  constructor(key, twk, twkmin, twkmax, radix, custom_radix_str) {
    if (verbose) { console.log(`FF1 key.length ${key.length}`) }
    if (custom_radix_str) {
  ...
  cipher(X, twk, encrypt) {
    ...
    if (verbose) { console.log(`cipher: twk: ${twk} ${!twk}  this.twk ${this.twk}`) }
    if (!twk || (Array.isArray(twk) && twk.length === 0)) {
      twk = this.twk;
      if (verbose) { console.log(`cipher: setting twk: ${twk}`) }
    }
