...
module.exports = class Encryption{
  ...
  begin(){
    ...
    let array_buf = struct.pack('!BBBBH', 0,0,this.algo['id'], this.iv.length, this.key['encrypted'].length)
    // Convert the array buffer into a string
    let encoded_str = array_buf.toString('binary');

    let struct_buf = new Buffer.from(encoded_str, "binary")
    ...
    let res = array_buf + this.iv + this.key['encrypted']
