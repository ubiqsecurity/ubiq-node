...
class Credentials {
  constructor(access_key_id, secret_signing_key, secret_crypto_access_key, host) {
    this.access_key_id = (access_key_id) || process.env.UBIQ_ACCESS_KEY_ID;
    this.secret_signing_key = (secret_signing_key) || process.env.UBIQ_SECRET_SIGNING_KEY;
    this.secret_crypto_access_key = (secret_crypto_access_key) || process.env.UBIQ_SECRET_CRYPTO_ACCESS_KEY;
...
class ConfigCredentials {
  ...
  load_credentials_file(credentials_file, profile) {
    ...
    const papi = (config.get(profile, 'ACCESS_KEY_ID')) ? config.get(profile, 'ACCESS_KEY_ID') : config.get('default', 'ACCESS_KEY_ID');
    const sapi = (config.get(profile, 'SECRET_SIGNING_KEY')) ? config.get(profile, 'SECRET_SIGNING_KEY') : config.get('default', 'SECRET_SIGNING_KEY');
    const srsa = (config.get(profile, 'SECRET_CRYPTO_ACCESS_KEY')) ? config.get(profile, 'SECRET_CRYPTO_ACCESS_KEY') : config.get('default', 'SECRET_CRYPTO_ACCESS_KEY');
