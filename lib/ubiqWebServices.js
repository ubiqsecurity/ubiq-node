...
class UbiqWebServices {
  constructor(ubiqCredentials, ubiqConfiguration) {
    ...
    // node-fetch changed in Node 19 to set keepAlive: true by default
    // This was causing problems with request handles were left open too long between cache TTL
    // and resulted in failed subsequent calls.  Changed default back to false and pass into the 
    // requests.
  ...
  async GetFfsConfigurationsync(datasetName) {
    const key = UrlHelper.GenerateFfsUrl(datasetName, this.ubiqCredentials);
  ...
  async GetFpeEncryptionKeyAsync(datasetName, keyNumber) {
    if (verbose) console.log(`UbiqWebServices GetFpeEncryptionKeyAsync: datasetName(${datasetName}) keyNumber(${keyNumber})`);
    if (verbose) console.log(`UbiqWebServices GetFpeEncryptionKeyAsync: ubiqCredentials(${this.ubiqCredentials}) keyNumber(${keyNumber})`);
    let key = '';
    if (keyNumber == null) {
      key = UrlHelper.GenerateFpeUrlEncrypt(datasetName, this.ubiqCredentials);
    } else {
      key = UrlHelper.GenerateFpeUrlDecrypt(datasetName, keyNumber, this.ubiqCredentials);
    }
    if (verbose) console.log(`UbiqWebServices GetFpeEncryptionKeyAsync: key(${key})`);

    if (verbose) console.log(`UbiqWebServices GetFpeEncryptionKeyAsync: ubiqCredentials(${(this.ubiqCredentials == null)})`);
    ...
    this.endpoint_base = `${this.baseUrl}${this.endpoint}`;
    if (verbose) console.log(`UbiqWebServices GetFpeEncryptionKeyAsync: endpoint_base(${this.endpoint_base})`);
    ...
    if (verbose) console.log(`UbiqWebServices GetFpeEncryptionKeyAsync: headers(${headers})`);
    ...
    if (verbose) console.log(`UbiqWebServices GetFpeEncryptionKeyAsync: otherParam(${otherParam})`);
    try {
      const response = await fetch(this.endpoint_base, otherParam);
      if (verbose) console.log(`UbiqWebServices GetFpeEncryptionKeyAsync: response(${response})`);
      if (response.status === 200) {
        const result = await response.json();
        if (verbose) console.log(`StructuredKeyCache GetAsync: result(${result})`);
        return result;
      }
      const apiError = await response.json();
      throw new Error(apiError.message);
    } catch (err) {
      if (verbose) console.log(`UbiqWebServices GetFpeEncryptionKeyAsync: err(${err})`);
  ...
  async GetFFSAndDataKeys(datasetName) {
    const key = UrlHelper.GenerateDefKeysUrl(datasetName, this.ubiqCredentials);
